...
// ============================================================
// דף כתבות - ניהול מאמרים
// ============================================================

const SAMPLE_ARTICLES = [
  {
    id: 'art-1', title: 'גוגל משדרגת את Chrome עם מילוי אוטומטי של מסמכי זיהוי, טיסות ועוד מ-Google Wallet',
    summary: 'גוגל הודיעה על העמקת השילוב בין שירות הארנק שלה לדפדפן הכרום במובייל ובדסקטופ.',
    author: 'יאל לכברמן', category: 'חדשות', categoryColor: '#1976D2', timestamp: 'היום, 11:20',
    imageUrl: 'https://images.unsplash.com/photo-1573804633927-bfcbcd909acd?w=400&q=80', link: ''
  },
  {
    id: 'art-2', title: 'הוכרז: Honor X80 Pro Max עם סוללת 11,000mAh ו-Snapdragon 6 Gen 5-i',
    summary: 'מותג הסמארטפונים הסיני הוכרז מציג מכשיר חדש בינוני גנטון קבולת סוללה יוצא דופן של 11,000 מיליאמפר-שעה.',
    author: 'רנן מנדז\'יצקי', category: 'חדשות', categoryColor: '#1976D2', timestamp: 'היום, 09:30',
    imageUrl: 'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=400&q=80', link: ''
  },
  {
    id: 'art-3', title: 'סקירה: Sony WH-CH720N – אוזניות מסננות רעשים בתקציב שפוי',
    summary: 'סוני מראה את הכוח בשוק הרעשים האקטיבי (ANC) גם לקטגוריית מחיר גמוכה מ-500 שקלים.',
    author: 'רנן מנדז\'יצקי', category: 'סקירות', categoryColor: '#7B1FA2', timestamp: 'לפני יומיים',
    imageUrl: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=400&q=80', link: ''
  },
  {
    id: 'art-4', title: 'מבצע סוף עונה: MacBook Air M3 במחיר חסר תקדים באילת',
    summary: 'דיל יום: אם אתם מתכננים ירידה לאילת בקרוב, רשתות השיווק המקומיות יוצאות במבצע ענק ללא מע"מ.',
    author: 'אורן מנרד', category: 'מבצעים', categoryColor: '#388E3C', timestamp: 'לפני 3 ימים',
    imageUrl: 'https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=400&q=80', link: ''
  }
];
...
function buildArticlesHTML(articles) {
  ...
  const featuredHTML = featured.map(a => `
    <div class="article-featured-card" onclick="${a.link ? `window.open('${a.link}','_blank')` : ''}">
      <img src="${a.imageUrl || 'https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=400&q=80'}" alt="${a.title}">
  ...
  const listHTML = rest.map(a => `
    <div class="article-row" data-art-id="${a.id}" onclick="${a.link ? `window.open('${a.link}','_blank')` : ''}">
      ...
      ${a.imageUrl ? `<img class="article-row-img" src="${a.imageUrl}" alt="${a.title}">` : ''}
      <button class="article-delete-btn" onclick="event.stopPropagation(); deleteArticleFromPage(this, '${a.id}')" title="מחק כתבה">✕</button>
    </div>
  `).join('');

  return `
    <div class="articles-page-container" data-articles='${JSON.stringify(articles).replace(/'/g, "&#39;")}'>
      <div class="articles-featured-grid">${featuredHTML}</div>
      <div class="articles-main-list">${listHTML}</div>
      <button class="add-article-btn" onclick="openArticleModal()">+ הוסף כתבה חדשה</button>
    </div>
  `;
}
...
function deleteArticleFromPage(btn, artId) {
  if (!isEditMode) return;
  const container = mainContent.querySelector('.articles-page-container');
  if (!container) return;
  let articles = JSON.parse(container.dataset.articles || '[]');
  articles = articles.filter(a => a.id !== artId);
  container.dataset.articles = JSON.stringify(articles);
  container.outerHTML = buildArticlesHTML(articles);
  saveCurrentPageContent();
}
...
if (btnAddArticlesPage) {
  btnAddArticlesPage.addEventListener('click', () => {
    const title = prompt('שם דף הכתבות:') || 'כתבות';
    const newPageId = 'page-' + Date.now();
    const html = buildArticlesHTML(SAMPLE_ARTICLES);
    pages.push({ id: newPageId, title: title.trim(), content: html });
    topNavPages.push(newPageId);
    activePageId = newPageId;
    saveToStorage();
    renderSideMenu();
...
document.getElementById('art-img-pick').addEventListener('click', () => {
  ...
  inp.onchange = e => {
    ...
    r.onload = ev => {
      artImageData = ev.target.result;
      const preview = document.getElementById('art-img-preview');
      preview.src = artImageData;
...
document.getElementById('art-save').addEventListener('click', () => {
  ...
  let articles = [];
  try { articles = JSON.parse(container.dataset.articles || '[]'); } catch(e) {}
  ...
  const newArt = {
    ...
    timestamp: 'עכשיו',
    imageUrl: artImageData || '',
    link: document.getElementById('art-link').value.trim()
  };
  articles.push(newArt);

  mainContent.innerHTML = buildArticlesHTML(articles);
  saveCurrentPageContent();
  document.getElementById('article-modal').style.display = 'none';
  artImageData = '';
