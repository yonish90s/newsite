...
// ============================================================
// מערכת כתבות (Article System)
// ============================================================

// ----- נתונים -----
// כל עמוד מכיל articles: [] שנשמר יחד עם pages

function getPageArticles(pageId) {
  const page = pages.find(p => p.id === pageId);
  if (!page) return [];
...
// ----- רינדור רשימת כתבות בתחתית הדף -----
function renderArticlesSection(pageId) {
  // מסיר קטע כתבות ישן אם קיים
  const old = document.getElementById('articles-section-' + pageId);
  if (old) old.remove();

  const articles = getPageArticles(pageId);
  if (articles.length === 0 && !isEditMode) return; // בצופה ריק - לא מציג כלום

  const section = document.createElement('div');
  section.className = 'articles-section';
  section.id = 'articles-section-' + pageId;
  if (isEditMode) section.classList.add('edit-mode-active');

  // כותרת
  const header = document.createElement('div');
  header.className = 'articles-section-header';
  const title = document.createElement('h2');
  title.className = 'articles-section-title';
  title.textContent = 'כתבות';
  header.appendChild(title);
  section.appendChild(header);

  if (articles.length === 0 && isEditMode) {
    const empty = document.createElement('p');
    empty.style.cssText = 'color:#aaa;font-size:14px;margin:0;';
    empty.textContent = 'לחץ על "הוסף כתבה" בסרגל הכלים כדי להוסיף את הכתבה הראשונה';
  ...
  articles.forEach(article => {
    ...
    // כפתורי עריכה (מופיעים רק במצב עריכה)
    const editBtns = document.createElement('div');
    editBtns.className = 'article-edit-btns';
    ...
    editBtn.onclick = (e) => {
      e.stopPropagation();
      openArticleEditor(pageId, article.id);
    };

    const deleteBtn = document.createElement('button');
    deleteBtn.className = 'article-edit-btn';
    deleteBtn.style.background = 'rgba(220,38,38,0.8)';
    deleteBtn.textContent = '🗑️ מחיקה';
    deleteBtn.onclick = (e) => {
      e.stopPropagation();
      if (confirm('למחוק את הכתבה "' + article.title + '"?')) {
        const page = pages.find(p => p.id === pageId);
        if (page) {
          page.articles = page.articles.filter(a => a.id !== article.id);
          saveToStorage();
          renderArticlesSection(pageId);
        }
      }
    };

    editBtns.appendChild(editBtn);
    editBtns.appendChild(deleteBtn);
    ...
    // לחיצה פותחת כתבה מלאה
    card.addEventListener('click', () => {
      if (isEditMode) return; // במצב עריכה לחיצה על הכרטיסייה לא מנווטת
      openArticleFullView(article, pageId);
    });

    section.appendChild(card);
  });

  // מחברים לאזור התוכן הראשי (אחרי התוכן הנוכחי)
  mainContent.appendChild(section);
}

// ----- תצוגת כתבה מלאה -----
let currentOpenArticlePageId = null;

function openArticleFullView(article, pageId) {
  currentOpenArticlePageId = pageId;

  document.getElementById('article-full-title').textContent = article.title;
  document.getElementById('article-full-author').textContent = article.author || '';
  document.getElementById('article-full-date').textContent = article.date || '';

  const imgEl = document.getElementById('article-full-image');
  if (article.imageUrl) {
  ...
  const bodyEl = document.getElementById('article-full-body');
  bodyEl.innerHTML = article.fullContent || '<p style="color:#aaa">הכתבה ריקה</p>';

  const pageName = pages.find(p => p.id === pageId);
  document.getElementById('article-full-page-name').textContent = pageName ? pageName.title : '';

  // כפתור עריכה מלאה (רק מנהל)
  const editBtn = document.getElementById('article-full-edit-btn');
  if (isEditMode) {
    editBtn.style.display = 'inline-block';
    editBtn.dataset.articleId = article.id;
    editBtn.onclick = () => {
      closeArticleFullView();
      setTimeout(() => openArticleEditor(pageId, article.id), 100);
    };
  } else {
...
function closeArticleFullView() {
  document.getElementById('article-full-view').classList.remove('open');
  document.body.style.overflow = '';
}

document.getElementById('article-back-btn').addEventListener('click', closeArticleFullView);

// ----- מודאל עריכת כתבה -----
let _articleEditorImageUrl = '';

function openArticleEditor(pageId, articleId) {
  const modal = document.getElementById('article-editor-modal');
  const page = pages.find(p => p.id === pageId);
  if (!page) return;
  if (!page.articles) page.articles = [];

  _articleEditorImageUrl = '';

  const heading = document.getElementById('article-editor-title-heading');
  document.getElementById('article-editor-id').value = articleId || '';
  document.getElementById('article-editor-title').value = '';
  document.getElementById('article-editor-excerpt').value = '';
  document.getElementById('article-editor-author').value = '';
  document.getElementById('article-editor-date').value = new Date().toLocaleDateString('he-IL');
  document.getElementById('article-editor-fullcontent').value = '';
  document.getElementById('article-editor-img-preview').style.display = 'none';

  if (articleId) {
    const article = page.articles.find(a => a.id === articleId);
    if (article) {
      heading.textContent = 'עריכת כתבה';
      document.getElementById('article-editor-title').value = article.title || '';
      document.getElementById('article-editor-excerpt').value = article.excerpt || '';
      document.getElementById('article-editor-author').value = article.author || '';
      document.getElementById('article-editor-date').value = article.date || '';
      document.getElementById('article-editor-fullcontent').value = article.fullContent || '';
      if (article.imageUrl) {
        _articleEditorImageUrl = article.imageUrl;
        const prev = document.getElementById('article-editor-img-preview');
        prev.src = article.imageUrl;
        prev.style.display = 'block';
      }
    }
  } else {
    heading.textContent = 'כתבה חדשה';
  }
...
function closeArticleEditor() {
  document.getElementById('article-editor-modal').classList.remove('open');
}

// כפתור ביטול
document.getElementById('article-editor-cancel').addEventListener('click', closeArticleEditor);

// כפתור בחירת תמונה
document.getElementById('article-editor-img-btn').addEventListener('click', () => {
  document.getElementById('article-editor-img-input').click();
});

document.getElementById('article-editor-img-input').addEventListener('change', (e) => {
  const file = e.target.files[0];
  if (!file) return;
  const reader = new FileReader();
  reader.onload = (ev) => {
    _articleEditorImageUrl = ev.target.result;
    const prev = document.getElementById('article-editor-img-preview');
    prev.src = _articleEditorImageUrl;
    prev.style.display = 'block';
  };
  reader.readAsDataURL(file);
});
...
// שמירת כתבה
document.getElementById('article-editor-save').addEventListener('click', () => {
  const title = document.getElementById('article-editor-title').value.trim();
  if (!title) {
    alert('חובה להזין כותרת לכתבה');
    return;
  }
  ...
  const articleId = document.getElementById('article-editor-id').value;
  const articleData = {
    id: articleId || 'article-' + Date.now(),
    title,
    excerpt: document.getElementById('article-editor-excerpt').value.trim(),
    author: document.getElementById('article-editor-author').value.trim(),
    date: document.getElementById('article-editor-date').value.trim(),
    fullContent: document.getElementById('article-editor-fullcontent').value,
    imageUrl: _articleEditorImageUrl
  };

  if (articleId) {
    const idx = page.articles.findIndex(a => a.id === articleId);
    if (idx > -1) page.articles[idx] = articleData;
    else page.articles.push(articleData);
  } else {
    page.articles.push(articleData);
  }

  saveToStorage();
  closeArticleEditor();
  renderArticlesSection(activePageId);
});

// ----- כפתור "הוסף כתבה" בסרגל הכלים -----
const btnAddArticle = document.getElementById('btn-add-article');
if (btnAddArticle) {
  btnAddArticle.addEventListener('click', () => {
    if (!isEditMode) {
      alert('יש לעבור למצב עריכה כדי להוסיף כתבה');
      return;
    }
    openArticleEditor(activePageId, null);
  });
}

// ----- שיפור renderPage: הוספת כתבות לאחר תוכן העמוד -----
const _origRenderPage = renderPage;
renderPage = function() {
  _origRenderPage();
  // מצרף כתבות לעמוד הנוכחי
  setTimeout(() => renderArticlesSection(activePageId), 0);
};
